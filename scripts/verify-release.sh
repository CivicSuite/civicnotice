...
find_python() {
  local candidates=()
  if [[ -n "${CIVICNOTICE_RELEASE_PYTHON:-}" ]]; then
    candidates+=("${CIVICNOTICE_RELEASE_PYTHON}")
  fi
...
exec((root / "civicnotice" / "__init__.py").read_text(encoding="utf-8"), namespace)
...
for path in [
    ...
    assert "0.1.3" in text, f"missing release version in {path}"
...
dist = Path("dist")
wheel = dist / "civicnotice-0.1.3-py3-none-any.whl"
sdist = dist / "civicnotice-0.1.3.tar.gz"
